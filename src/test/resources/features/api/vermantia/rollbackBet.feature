...
Функционал: API-vermantia
  ...
  @vermantia
  @cancelbet
  Сценарий: Непринятая ставка vermantia на рубли
    * сохраняем в память
      | CURRENCY  | RUB |
...
#запоминаем баланс
    ...
    * проверим что "HOLDRUB2" больше "HOLDRUB" на "AMOUNT"
    ...
    #отменяем ставку
    * запрос по адресу "{ESB_URL}/rest/webrollbackbet" и сохраняем в "RESPONCE_API":
      | group_code  | GROUPCODE |
      | shop_code   | SHOPCODE  |
      | session     | SESSION   |
      | skin        | main      |
    ...
    * проверим что "HOLDRUB2" больше "HOLDRUB" на "0"
    ...
    #проверям баланс пользователя. он должен был снова стать как в началае - деньги вернулись
    * запрос типа GET, результат сохраняем в "RESPONCE"
      | data   | ESB_URL              |
      | string | rest/webuserbalance  |
      | stash  | GROUPCODE            |
    ...
    * находим "balance" и сохраняем "BALANCE2" из "RESPONCE"
    * проверим что "BALANCE" больше "BALANCE2" на "0"


  @api
  @vermantia
  @cancelbet
  Сценарий: Непринятая ставка vermantia на бонусы
    * сохраняем в память
      | CURRENCY  | RUB |
...
#запоминаем баланс
    ...
    * находим "bonus-balance" и сохраняем "BONUSES2" из вложенного "RESPONCE"
    * проверим что "BONUSES" больше "BONUSES2" на "0"
