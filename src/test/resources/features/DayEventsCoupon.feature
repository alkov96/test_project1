# language: ru
  Функционал: добавление события в купон на странице События Дня. Сравнение коэффицента и суммы в купоне, и на баннере и на странице. Проверка соответствия вида спорта и событий в нём.
    @smoke
    @DayEventsCoupon
      Сценарий: Проверка купона и событий на странице События Дня
      ...
      * пользователь (проверяет, совпадают ли события на баннере с событиями в купоне)
      * пользователь (проверяет, совпадает ли исход в купоне с выбранным на баннере)
